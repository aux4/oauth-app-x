...
```execute
aux4 oauth-app-x authorize-url
```
...
```execute
aux4 oauth-app-x authorize-url --params '{"provider":"github"}'
```
...
```execute
X_CLIENT_ID= X_CLIENT_SECRET= aux4 oauth-app-x authorize-url --params '{"provider":"x"}' --query '{"redirectUri":"http://127.0.0.1:9876/callback"}'
```
...
```execute
X_CLIENT_ID=test-client-id aux4 oauth-app-x authorize-url --params '{"provider":"x"}' --query '{"redirectUri":"http://127.0.0.1:9876/callback","scopes":"tweet.read users.read offline.access","state":"xyz"}'
```
...
```execute
X_CLIENT_ID=test-client-id aux4 oauth-app-x authorize-url --params '{"provider":"x"}' --query '{"redirectUri":"http://127.0.0.1:9876/callback"}'
```
